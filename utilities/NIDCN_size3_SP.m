function im_h_y = NIDCN_size3_SP(im_l_y,model)
...
convfea_1 = vl_nnconv(im_y,weight{7},bias{7},'Pad',1);

convfea = vl_nnconv(convfea_1,weight{1},bias{1},'Pad',1);
...
convfea = vl_nnconv(convfea,weight{5},bias{5},'Pad',1);
%disp(size(convfea));
...
im_h_y = vl_nnconv(im_h_y_1,weight{6},bias{6},'Pad',1);
 %disp(size(im_h_y)); 
