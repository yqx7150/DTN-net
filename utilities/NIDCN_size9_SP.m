function im_h_y = NIDCN_size9_SP(im_l_y,model)
...
convfea_1 = vl_nnconv(im_y,weight{7},bias{7},'Pad',4);

convfea = vl_nnconv(convfea_1,weight{1},bias{1},'Pad',4);
convfea = vl_nnrelu(convfea);

%disp(size(convfea));

convfea = vl_nnconv(convfea,weight{2},bias{2},'Pad',4);
convfea = vl_nnrelu(convfea);
...
convfea = vl_nnconv(convfea,weight{3},bias{3},'Pad',4);
convfea = vl_nnrelu(convfea);

%disp(size(convfea));

convfea = vl_nnconv(convfea,weight{4},bias{4},'Pad',4);
convfea = vl_nnrelu(convfea);

%disp(size(convfea));
 
convfea = vl_nnconv(convfea,weight{5},bias{5},'Pad',4);
%disp(size(convfea));
...
im_h_y = vl_nnconv(im_h_y_1,weight{6},bias{6},'Pad',4);
 %disp(size(im_h_y)); 
